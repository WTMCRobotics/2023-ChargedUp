...
public class Vision {
  ...
  public void start() {
    ...
    m_visionThread = new Thread(() -> {
      ...
      /**
       * This is the closest AprilTag, we will now hone in on it. We will declare this later on
       */
      int targetedAprilTag = -1;
      /**
       * This will count how many frames in a row the closest april tag is out of frames for, if it
       * reaches a threshold, it will find a new april tag.
       */
            ...
            if (aprilConfidence.get(loopAprilTag.getId()) == 0) {
              ...
              SmartDashboard.putString("April Tag " + loopAprilTag.getId(),
                  ...
                  "Center: " + df.format(loopAprilTag.getCenterX()) + ", "
                      + df.format(loopAprilTag.getCenterY()) + " AprilTag ID: "
                      + loopAprilTag.getId()
              // }
              );
        ...
        for (int i = 0; i < 29; i++) {
          ...
          if (!hasDetectedLastFrame.get(i)) {
            ...
            if (aprilConfidence.get(i) > 1) {
              ...
              if (aprilConfidence.get(i) < 1)
                // If not, remove it from the dashboard, as it's probably not in view anymore
  ...
  private String neededRotation(AprilTagDetection tag) {
    ...
    if (lineLengthDifference >= -ROTATION_MARGIAN && lineLengthDifference <= ROTATION_MARGIAN) {
      ...
      // This can be replaced by a -1 for a more machine readable for, also, I have no idea if this
      // is left or right, just a placeholder
      return "Left";
    } else {
      // AKA. 1, this might also be wrong becaise at the time of writing, I can't test whether it is
      // left or right
      return "Right";
    }
  }
