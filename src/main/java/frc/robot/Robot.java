...
public class Robot extends TimedRobot {
    ...
    public void robotInit() {
        ...
        frontLeft = MotorControllerFactory.create(this, 30, MotorController.Type.Talon);
    ...
    public double getMotorPower(double x, double y, double turnAmount, double scaleDownFactor,
        ...
        if (slowModeEnabled) {
            // If slow mode is enabled, increase the scale factor by 4, which will make it frice as
            // slow
            scaleDownFactor *= 4;
        }
        switch (motorId) {
    ...
    public void teleopPeriodic() {
        ...
        // The left joystick values will determine the direction that the robot will strafe
        // (crabwalk), these values will not rotate the robot
        ...
        // * 1.1 we might need to multiply by a bit more than one, if we want to counteract
        // imperpefect strafing
        double leftXboxJoystickX = -xboxController.getLeftX();
        ...
        // This value determines how fast we should strafe, it is gotten by doing the pythagoras
        // theorem
        ...
        double magnitude =
                Math.sqrt((Math.pow(leftXboxJoystickX, 2)) + (Math.pow(leftXboxJoystickY, 2)));


        /*
         * The scale factor is the number we need to divide everything thing by to ensure all of our
         * values are less than 1 while keeping the same ratio. For example, say we had motor powers
         * of 0.9, 1.0, 1.1, and 1.2. The top 3 values would all be the same, as motor power can
         * only go up 1. The scale factor factor makes sure that the values all keep the same ratio
         * (in this example: 9:10:11:12). It does this by making the largest value equal to 1, and
         * scaling the other values down proportionally
         */
        double scaleDownFactor = Math.max(
                Math.abs(leftXboxJoystickY) + Math.abs(leftXboxJoystickX) + Math.abs(turnAmount),
                1);
        // to appease Colin:
        // double
        // scaleFactorNeededToMultipleyeverythingByToEnsureThatEveryhingIsLessThanOneWhileStillMaintainingProportionalToEachOther
        // = Math.max(Math.abs(leftXboxJoystickY) + Math.abs(leftXboxJoystickX) +
        // Math.abs(turnAmount), 1);
        ...
        double frontLeftPower = getMotorPower(leftXboxJoystickX, leftXboxJoystickY, turnAmount,
                scaleDownFactor, slowMode, Constants.FRONT_LEFT_MOTOR_ID);
        double frontRightPower = -getMotorPower(leftXboxJoystickX, leftXboxJoystickY, turnAmount,
                scaleDownFactor, slowMode, Constants.FRONT_RIGHT_MOTOR_ID);
        double backRightPower = -getMotorPower(leftXboxJoystickX, leftXboxJoystickY, turnAmount,
                scaleDownFactor, slowMode, Constants.BACK_RIGHT_MOTOR_ID);
        double backLeftPower = getMotorPower(leftXboxJoystickX, leftXboxJoystickY, turnAmount,
                scaleDownFactor, slowMode, Constants.BACK_LEFT_MOTOR_ID);
